...
// Relaunch ourselves as a GUI process so the browser-native host (which must
// stay headless and keep its stdin/stdout for frames) can hand the link off.
...
// and the origin form both land here.
...
int SelfTest::runMultiSegmentTest() {
    try {
...
// Speed limiter test: 10 MB (â‰ˆ6.7 s at 1.5 MiB/s) must actually take that
// long and never average faster than the cap.
int SelfTest::runSpeedLimitTest() {
    const std::string url = "https://proof.ovh.net/files/10Mb.dat";
    const std::int64_t expected = 10LL * 1024 * 1024;
...
int SelfTest::runNativeHostTest() {
    int argc = 1;
    char prog[] = "phoenix";
    char* argv[] = {prog};
    QCoreApplication app(argc, argv);
